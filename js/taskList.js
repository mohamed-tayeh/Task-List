(function () {
  ...
  // Class and IDs
  const taskContainerId = 'task-container';
  const titleId = 'title';
  const taskDivClass = 'task-div';
  const taskSenderClass = 'sender';
  const taskContentClass = 'task-content';
  const cloneLabel = 'clone';
  const dataFile = 'tasksTracker';

  // Number of tasks supported
  const pxCutOffLength = 475;
  const limitTaskNum = 18;
  const taskDivHeight = 26;
  ...
  window.addEventListener('load', function () {
    ...
    function getTasksTracker() {
      return JSON.parse(localStorage.getItem(dataFile));
    }

    function updateTasksTracker(tasksTracker) {
      localStorage.setItem(dataFile, JSON.stringify(tasksTracker));
    }

    let tasksTracker = getTasksTracker();
    updateTasksTracker(null);

    const taskContainerEl = document.getElementById(taskContainerId);
    ...
    function trimCharLength(senderId) {
      ...
      const targetLength = pxCutOffLength - senderIdLength;
    ...
    function addTask(senderId, taskContent) {
      ...
      if (length > limitTaskNum && duplicatesAdded) {
        let lastElDuplicate = document.getElementById(
          idsList[length - 1] + cloneLabel
        );

        let taskDivDuplicate = taskDiv.cloneNode(true);
        taskDivDuplicate.id = senderId + cloneLabel;
        taskDivDuplicate.classList.add(cloneLabel);
    ...
    function editTask(senderId, taskContent) {
      if (tasksMap.hasOwnProperty(senderId)) {
        let taskDiv = document.getElementById(senderId);
        let taskContentDiv = taskDiv.querySelectorAll(`.${taskContentClass}`);
    ...
    function checkForLooping() {
      ...
      if (length > limitTaskNum && !duplicatesAdded) {
        ...
        startLooping();
      } else if (length <= limitTaskNum && duplicatesAdded) {
        stopLooping();
      }
    }
    ...
    function startLooping() {
      addDuplicates();
      document.getElementById(taskContainerId).style.animation =
    ...
    function addDuplicates() {
      Object.keys(tasksMap).forEach((senderId) => {
        ...
        taskDiv.id = senderId + cloneLabel;
        taskDiv.classList.add(cloneLabel);
        taskContainerEl.appendChild(taskDiv);
      });
    }
    ...
    function editDuplicate(senderId, taskContent) {
      ...
      if (isLooping) {
        let taskDiv = document.getElementById(senderId + cloneLabel);
        let taskContentDiv = taskDiv.querySelectorAll(`.${taskContentClass}`);
        taskContentDiv[0].innerHTML = taskContent;
      }
    }
